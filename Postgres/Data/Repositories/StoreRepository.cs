...
{
    public StoreRepository(TinyBaseDbContext context) : base(context)
    {
    }
    ...
    public async Task<IEnumerable<Store>> GetPersistedStoresAsync(CancellationToken cancellationToken = default)
    {
        return await _dbSet.Where(s => s.IsPersisted).ToListAsync(cancellationToken);
    }

    public async Task<Store> UpdateConfigurationAsync(string storeId, string configuration, string configHash, CancellationToken cancellationToken = default)
    {
